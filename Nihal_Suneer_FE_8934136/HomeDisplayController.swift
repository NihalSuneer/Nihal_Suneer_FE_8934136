...
class HomeDisplayController: UIViewController, CLLocationManagerDelegate {
       ...
       if let tabBarController = self.tabBarController,
          ...
          index < viewControllers.count {
           ...
           if let navController = tabBarController.viewControllers?[index] as? UINavigationController {
               ...
               if let topViewController = navController.topViewController as? NewsController {
                   topViewController.setCityName(cityName)
               }
           } else if let destinationVC = viewControllers[index] as? DirectionController {
               destinationVC.setCityName(cityName)
           } else if let destinationVC = viewControllers[index] as? WeatherController {
               destinationVC.setCityName(cityName)
           }
       }
    }
